...
// // Cache TTl in seconds(7days)

// const CACHE_TTL = 7 * (24 * 60 * 60);
// const MAX_CACHE_SIZE = 1024000;

// export const urlPattern =
//   /https?:\/\/(www\.)?[-a-zA-Z0-9%.+~]{1,256}\.[a-zA-Z]{2,6}\b([-a-zA-Z0-9@:%+.~#?&/=]*)/;
...
//   );
// }
// // Function to get cache key for a URL with sanitizationn
...
//     // Set expiration to 7 days (in seconds)
//     await redis.expire(`conversation:${id}`, 7 * 24 * 60 * 60);

//     logger.info(`Successfully saved conversation ${id} with ${messages.length} messages`);
//   } catch (error) {
...
//       logger.info(`Successfully retrieved conversation ${id} with ${messages.length} messages`);
//       return messages;
//     }

//     return null; // Fallback if data is not a string, although unlikely
//   } catch (error) {
...
export async function saveConversation(id: string, messages: Message[]) {
  try {
    ...
    await redis.set(`conversation:${id}`, JSON.stringify(messages));
    await redis.expire(`conversation:${id}`, CACHE_TTL);
